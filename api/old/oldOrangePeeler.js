...
function teamOffensiveInfluencePeeler(scheduleCollection, matchDataCollection, callback){
	//This make sure to find the oranges that are important for calculating OPR; schedule and matchdata
	console.log('[START]-teamOffensiveInfluencePeeler')
	MongoClient.connect('mongodb://localhost/TheOrangeAllianceTest', function(err,db){
		...
		function teamsScores(err, docs){
			if(err){
				...
				console.log('Operation teamOffensiveInfluencePeeler time(Milliseconds):',new Date(new Date()-peelerTimer).getMilliseconds())
				console.log('[DONE]-teamOffensiveInfluencePeeler')
				db.close()
				callback(peeledOrange)
			}
		}
	})
}

function teamMarginalInfluencePeeler(scheduleCollection, matchDataCollection, callback){
	//This make sure to find the oranges that are important for calculating OPR; schedule and matchdata
				...
				teamsScore:[
					{
						...
						team1:'$schedule.teams.red1',
						team2:'$schedule.teams.red2',
						score:{$subtract:['$matchData.resultsInformation.score.total.red', '$matchData.resultsInformation.score.total.blue']}
					},
					{
						//teams:['$schedule.teams.blue1','$schedule.teams.blue2'],
						team1:'$schedule.teams.blue1',
						team2:'$schedule.teams.blue2',
						score:{$subtract:['$matchData.resultsInformation.score.total.blue', '$matchData.resultsInformation.score.total.red']}
					}
				]
			}},
			{$unwind:'$teamsScore'},
...
//teamOffensiveInfluencePeeler('scheduleturing', 'matchDataturing')
//teamOffensiveInfluencePeeler('scheduleeuclid', 'matchDataeuclid')
//teamMarginalInfluencePeeler('scheduleeuclid', 'matchDataeuclid')
//teamMarginalInfluencePeeler('scheduleturing', 'matchDataturing', callback)

function callback(peeledOrange){
	console.log(peeledOrange)
}

module.exports = {
	teamOffensiveInfluencePeeler: teamOffensiveInfluencePeeler,
	teamMarginalInfluencePeeler: teamMarginalInfluencePeeler
}